...
void startTimer();
double getTimeElasped(); // in millisecond 
  ...
  for(;;)
  {
    ...
    switch(c)
    {
      case '1'://    "1: start record image by RQ_GetImage\n"
        {
          ...
          }else
          {
            ...
            {
              printf("succeed to create thread_RecordImage\n");
            }
          }
          break;
        }
      case '2'://    "2: RQ_GetVersion\n"
        {
          // stop the overo Server first
          IPObj tx;
          tx.id = ZH_ImageStreamStop;  // start the ImageStreamStart
          tx.size=sizeof(tx);
  
          // start ImageTransferStart
          res=send(s,(char*)&tx,sizeof(tx),0);  
          printf("sent %d bytes, stop imageTransfer\n",res);
...
void* thread_RecordImage(void* p)
{
  ...
  while(g_thread_record_on) // sym
  {
    ...
    if(data)
    {
      ...
      // cancel confirm msg
      // printf("TCPRecv consume %d, send confirm info\n",res);
      // send(*ps, (char*)&hx, sizeof(hx), 0); // send confirm msg
      ...
      // sprintf(FName, "imgs/frame_%d.bin", frame_num++);
       ...
       printf("Host computer receive %d frames with %d bytes\n", frame_num - 1, res);
      
       /*
      if(n_count == 0)
      {
        startTimer();
      }else if(n_count == N_frames_-1)
      {
        double ms = getTimeElasped();
        printf("collect %d frames using %lf ms, fps is %.2lf\n", N_frames_, ms,((double)N_frames_*1000.)/(ms));
        n_count = -1; 
      }
      ++n_count;
      */
...
{
  gettimeofday(&g_et, 0);
  return (double)((g_et.tv_sec - g_st.tv_sec)*1000 + (g_et.tv_usec - g_st.tv_usec)/1000);
