...
CSReader::CSReader()
{
  ...
  nh.param("sr_data_file_dir", file_dir_, string("/home/davidz/work/data/SwissRanger4000/exp/dataset_1")); // default parameters 
...
bool CSReader::loadAllData()
{
  ...
  for(int i=start_frame_, j=0; i<end_frame_ && j<specific_id.size(); i++, j++)
  {
    ...
    // if(!b_sr_new_version_)
    {
      // old file version 
      if(b_sr_specific_id_)
         ss<<file_dir_<<"/"<<data_prefix_<<"_"<<setfill('0')<<setw(4)<<start_frame_+specific_id[j]<<"."<<data_suffix_; 
      else
         ss<<file_dir_<<"/"<<data_prefix_<<"_"<<setfill('0')<<setw(4)<<i<<"."<<data_suffix_; 
    }
    // else{
