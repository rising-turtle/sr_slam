...
// #include "DepthEstimation/CDepthMap.h"
...
CDepthMapSE3::CDepthMapSE3(int w, int h, const Eigen::Matrix3f& K)
{
	width = w;
	height = h;
...
void CDepthMapSE3::observeDepthRow(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMapSE3::observeDepth()
{

	threadReducer.reduce(boost::bind(&CDepthMapSE3::observeDepthRow, this, _1, _2, _3), 3, height-3, 10);
...
bool CDepthMapSE3::makeAndCheckEPL(const int x, const int y, const FrameSE3* const ref, float* pepx, float* pepy, RunningStats* const stats)
{
	int idx = x+y*width;
...
bool CDepthMapSE3::observeDepthCreate(const int &x, const int &y, const int &idx, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;
...
bool CDepthMapSE3::observeDepthUpdate(const int &x, const int &y, const int &idx, const float* keyFrameMaxGradBuf, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;
	FrameSE3* refFrame;
...
void CDepthMapSE3::propagateDepth(FrameSE3* new_keyframe)
{
	...
	for(DepthMapPixelHypothesis* pt = otherDepthMap+width*height-1; pt >= otherDepthMap; pt--)
	{
        ...
        // assign from new_keyframe, if it has depth 
        if(new_keyframe->hasIDepthBeenSet())
        {
          const float* idepth = new_keyframe->idepth(); 
            ...
            for(int x=0; x<width; x++)
            {
              ...
              if(!isnanf(idepthValue) && idepthValue > 0)
              {
                otherDepthMap[x+y*width] = DepthMapPixelHypothesis(
                    idepthValue, idepthValue, VAR_GT_INIT_INITIAL, 
                    VAR_GT_INIT_INITIAL, 20);
              }
            }
          }
        }
...
void CDepthMapSE3::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMapSE3::regularizeDepthMapFillHoles()
{
	...
	threadReducer.reduce(boost::bind(&CDepthMapSE3::regularizeDepthMapFillHolesRow, this, _1, _2, _3), 3, height-2, 10);
...
void CDepthMapSE3::buildRegIntegralBufferRow1(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMapSE3::buildRegIntegralBuffer()
{
	threadReducer.reduce(boost::bind(&CDepthMapSE3::buildRegIntegralBufferRow1, this, _1, _2,_3), 0, height);
...
template<bool removeOcclusions> void CDepthMapSE3::regularizeDepthMapRow(int validityTH, int yMin, int yMax, RunningStats* stats)
{
...
template void CDepthMapSE3::regularizeDepthMapRow<true>(int validityTH, int yMin, int yMax, RunningStats* stats);
template void CDepthMapSE3::regularizeDepthMapRow<false>(int validityTH, int yMin, int yMax, RunningStats* stats);


void CDepthMapSE3::regularizeDepthMap(bool removeOcclusions, int validityTH)
{
	...
	if(removeOcclusions)
		threadReducer.reduce(boost::bind(&CDepthMapSE3::regularizeDepthMapRow<true>, this, validityTH, _1, _2, _3), 2, height-2, 10);
	else
		threadReducer.reduce(boost::bind(&CDepthMapSE3::regularizeDepthMapRow<false>, this, validityTH, _1, _2, _3), 2, height-2, 10);
...
void CDepthMapSE3::initializeRandomly(FrameSE3* new_frame)
{
...
void CDepthMapSE3::setFromExistingKF(FrameSE3* kf)
{
...
void CDepthMapSE3::initializeFromGTDepth(FrameSE3* new_frame)
{
...
void CDepthMapSE3::resetCounters()
{
...
void CDepthMapSE3::updateKeyframe(std::deque< std::shared_ptr<FrameSE3> > referenceFrames)
{
	assert(isValid());
...
void CDepthMapSE3::createKeyFrame(FrameSE3* new_keyframe)
{
	assert(isValid());
	...
	for(DepthMapPixelHypothesis* source = currentDepthMap; source < currentDepthMap+width*height; source++)
	{
        ...
        // for depth given map, no scale is needed 
        /*
...
void CDepthMapSE3::addTimingSample()
{
	struct timeval now;
...
void CDepthMapSE3::finalizeKeyFrame()
{
	assert(isValid());
...
int CDepthMapSE3::debugPlotDepthMap()
{
...
inline float CDepthMapSE3::doLineStereo(
