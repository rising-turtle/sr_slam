...
int main(int argc, char* argv[])
{
  ...
  while(!finished)
  {
    try{
      ...
      // cv::bilateralFilter(img, blur_img, 5, 2, 2);

      // threshold_label2(blur_img, 25, 1);

      // undistorter->undistort(blur_img, rect_img); 
      ...
      if(runningIDX == 0)
          ...
          system->trackFrame(he_img.data, (float*)(rect_dpt.data), runningIDX, 1, fakeTimeStamp); 
      runningIDX++; 
      // fakeTimeStamp += 0.03;
