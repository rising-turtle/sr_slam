...
CDepthMap::CDepthMap(int w, int h, const Eigen::Matrix3f& K)
{
	width = w;
	height = h;
...
void CDepthMap::observeDepthRow(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMap::observeDepth()
{

	threadReducer.reduce(boost::bind(&CDepthMap::observeDepthRow, this, _1, _2, _3), 3, height-3, 10);
...
bool CDepthMap::makeAndCheckEPL(const int x, const int y, const Frame* const ref, float* pepx, float* pepy, RunningStats* const stats)
{
	int idx = x+y*width;
...
bool CDepthMap::observeDepthCreate(const int &x, const int &y, const int &idx, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;
...
bool CDepthMap::observeDepthUpdate(const int &x, const int &y, const int &idx, const float* keyFrameMaxGradBuf, RunningStats* const &stats)
{
	DepthMapPixelHypothesis* target = currentDepthMap+idx;
	Frame* refFrame;
...
void CDepthMap::propagateDepth(Frame* new_keyframe)
{
...
void CDepthMap::regularizeDepthMapFillHolesRow(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMap::regularizeDepthMapFillHoles()
{
	...
	threadReducer.reduce(boost::bind(&CDepthMap::regularizeDepthMapFillHolesRow, this, _1, _2, _3), 3, height-2, 10);
...
void CDepthMap::buildRegIntegralBufferRow1(int yMin, int yMax, RunningStats* stats)
{
...
void CDepthMap::buildRegIntegralBuffer()
{
	threadReducer.reduce(boost::bind(&CDepthMap::buildRegIntegralBufferRow1, this, _1, _2,_3), 0, height);
...
template<bool removeOcclusions> void CDepthMap::regularizeDepthMapRow(int validityTH, int yMin, int yMax, RunningStats* stats)
{
...
template void CDepthMap::regularizeDepthMapRow<true>(int validityTH, int yMin, int yMax, RunningStats* stats);
template void CDepthMap::regularizeDepthMapRow<false>(int validityTH, int yMin, int yMax, RunningStats* stats);


void CDepthMap::regularizeDepthMap(bool removeOcclusions, int validityTH)
{
	...
	if(removeOcclusions)
		threadReducer.reduce(boost::bind(&CDepthMap::regularizeDepthMapRow<true>, this, validityTH, _1, _2, _3), 2, height-2, 10);
	else
		threadReducer.reduce(boost::bind(&CDepthMap::regularizeDepthMapRow<false>, this, validityTH, _1, _2, _3), 2, height-2, 10);
...
void CDepthMap::initializeRandomly(Frame* new_frame)
{
...
void CDepthMap::setFromExistingKF(Frame* kf)
{
...
void CDepthMap::initializeFromGTDepth(Frame* new_frame)
{
...
void CDepthMap::updateKeyframe(std::deque< std::shared_ptr<Frame> > referenceFrames)
{
	assert(isValid());
...
void CDepthMap::createKeyFrame(Frame* new_keyframe)
{
	assert(isValid());
...
void CDepthMap::addTimingSample()
{
	struct timeval now;
...
void CDepthMap::finalizeKeyFrame()
{
	assert(isValid());
...
int CDepthMap::debugPlotDepthMap()
{
...
inline float CDepthMap::doLineStereo(
