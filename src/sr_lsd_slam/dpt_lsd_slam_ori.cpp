...
CDLSDSlamSystemOri::CDLSDSlamSystemOri(int w, int h, Eigen::Matrix3f K, bool enableSLAM)
: SLAMEnabled(enableSLAM), relocalizer(w,h,K)
{
	...
	thread_mapping = boost::thread(&CDLSDSlamSystemOri::mappingThreadLoop, this);

	if(SLAMEnabled)
	{
		// thread_constraint_search = boost::thread(&CDLSDSlamSystemOri::constraintSearchThreadLoop, this);
	        thread_constraint_search = boost::thread(&CDLSDSlamSystemOri::constraintSearchThreadLoop_simple, this);
		thread_optimization = boost::thread(&CDLSDSlamSystemOri::optimizationThreadLoop, this);
	}
...
CDLSDSlamSystemOri::~CDLSDSlamSystemOri()
{
	keepRunning = false;

	// make sure none is waiting for something.
	printf("... waiting for CDLSDSlamSystemOri's threads to exit\n");
	...
	printf("DONE waiting for CDLSDSlamSystemOri's threads to exit\n");
...
void CDLSDSlamSystemOri::setVisualization(Output3DWrapper* outputWrapper)
{
	this->outputWrapper = outputWrapper;
}

void CDLSDSlamSystemOri::mergeOptimizationOffset()
{
...
void CDLSDSlamSystemOri::mappingThreadLoop()
{
...
void CDLSDSlamSystemOri::finalize()
{
...
void CDLSDSlamSystemOri::constraintSearchThreadLoop_simple()
{
...
void CDLSDSlamSystemOri::findConstraintsForNewKeyFrames_simple(Frame* newKeyFrame, bool forceParent, bool useFABMAP, float closeCandidatesTH)
{
  ...
  Frame* p_parent = 0; // parent->hasTrackingParent()?parent->getTrackingParent(): 0;
  ...
  Sim3 candidateToFrame_initialEstimate = newKeyFrame->getScaledCamToWorld().inverse() * parent->getScaledCamToWorld();
...
/*
void CDLSDSlamSystemOri::testConstraintSE3(
    Frame* candidate, KFConstraintStruct* &e1_out, KFConstraintStruct* &e2_out, 
...
float CDLSDSlamSystemOri::tryTrackSE3(
    ...
    KFConstraintStruct *e1, KFConstraintStruct* e2)
{
    ...
    AtoB = constraintSE3Tracker->trackFrame(
        B, A->keyframe, AtoB, lvlStart, lvlEnd); 
...
*/
void CDLSDSlamSystemOri::constraintSearchThreadLoop()
{
...
void CDLSDSlamSystemOri::optimizationThreadLoop()
{
...
void CDLSDSlamSystemOri::publishKeyframeGraph()
{
	if (outputWrapper != nullptr)
		outputWrapper->publishKeyframeGraph(keyFrameGraph);
}

void CDLSDSlamSystemOri::requestDepthMapScreenshot(const std::string& filename)
{
	depthMapScreenshotFilename = filename;
	depthMapScreenshotFlag = true;
}

void CDLSDSlamSystemOri::finishCurrentKeyframe()
{
...
void CDLSDSlamSystemOri::discardCurrentKeyframe()
{
	if(enablePrintDebugInfo && printThreadingInfo)
...
void CDLSDSlamSystemOri::createNewCurrentKeyframe(std::shared_ptr<Frame> newKeyframeCandidate)
{
	if(enablePrintDebugInfo && printThreadingInfo)
...
void CDLSDSlamSystemOri::loadNewCurrentKeyframe(Frame* keyframeToLoad)
{
	if(enablePrintDebugInfo && printThreadingInfo)
...
void CDLSDSlamSystemOri::changeKeyframe(bool noCreate, bool force, float maxScore)
{
...
bool CDLSDSlamSystemOri::updateKeyframe()
{
...
void CDLSDSlamSystemOri::addTimingSamples()
{
	map->addTimingSample();
	struct timeval now;
...
void CDLSDSlamSystemOri::debugDisplayDepthMap()
{
...
void CDLSDSlamSystemOri::takeRelocalizeResult()
{
...
bool CDLSDSlamSystemOri::doMappingIteration()
{
...
void CDLSDSlamSystemOri::gtDepthInit(uchar* image, float* depth, double timeStamp, int id)
{
...
void CDLSDSlamSystemOri::randomInit(uchar* image, double timeStamp, int id)
{
...
void CDLSDSlamSystemOri::trackFrame(uchar* image, float* dpt, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	// Create new frame
...
float CDLSDSlamSystemOri::tryTrackSim3(
		TrackingReference* A, TrackingReference* B,
		int lvlStart, int lvlEnd,
		bool useSSE,
		Sim3 &AtoB, Sim3 &BtoA,
...
void CDLSDSlamSystemOri::testConstraint(
		Frame* candidate,
...
int CDLSDSlamSystemOri::findConstraintsForNewKeyFrames(Frame* newKeyFrame, bool forceParent, bool useFABMAP, float closeCandidatesTH)
{
	if(!newKeyFrame->hasTrackingParent())
	{
		newConstraintMutex.lock();
...
bool CDLSDSlamSystemOri::optimizationIteration(int itsPerTry, float minChange)
{
	struct timeval tv_start, tv_end;
	gettimeofday(&tv_start, NULL);
...
void CDLSDSlamSystemOri::optimizeGraph()
{
...
SE3 CDLSDSlamSystemOri::getCurrentPoseEstimate()
{
	SE3 camToWorld = SE3();
...
std::vector<FramePoseStruct*, Eigen::aligned_allocator<FramePoseStruct*> > CDLSDSlamSystemOri::getAllPoses()
{
